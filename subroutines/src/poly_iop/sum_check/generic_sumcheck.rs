...
impl<F: PrimeField> ZerocheckInstanceProof<F> {
        ...
        Func: Fn(&[F]) -> F + std::marker::Sync,
    {
        ...
        let accum: Vec<Vec<F>> = (0..mle_half)
            ...
            .map(|poly_term_i| {
                ...
                // Evaluate P({0, ..., |g(r)|})
                let eq_eval = eq_table[poly_term_i];
                ...
                accum[0] += comb_func(&params_zero) * eq_eval;
                ...
                for eval_i in 2..(combined_degree + 1) {
                    ...
                    accum[eval_i] += comb_func(&poly_evals) * eq_eval;
        ...
        Func: Fn(&[F]) -> F + std::marker::Sync,
    {
        ...
        for round in 0..num_rounds {
            let mut eval_points =
                Self::prove_arbitrary_step(polys, &comb_func, &eq_table[round], combined_degree);
            eval_points[1] = zerocheck_r_inv[round]
                * (previous_claim - (F::one() - zerocheck_r[round]) * eval_points[0]);
        ...
        (ZerocheckInstanceProof::new(proof_polys), r, final_evals)
    }
        ...
        Func: Fn(&[F]) -> F + std::marker::Sync,
    {
        ...
        Some((ZerocheckInstanceProof::new(proof_polys), r, final_evals))
    }
